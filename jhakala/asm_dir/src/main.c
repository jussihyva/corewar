...
/*   Updated: 2020/08/10 18:42:07 by jhakala          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		*fill(char *str, int *op)
{
	...
	if (check == 1)
		return (o_error("usage: ./asm [-fhed] source_file\n", op));
	i = 0;
	while (str[++i])
		op[122 - str[i]] = 1;
	return (op);
...
/*
** use ./asm -d source_file.s for saving to asm directory,
** without -d, asm will save it to original .s directory.
** -h: usage
** -e: multiple errors
** -f: leaks
*/
...
int		main(int ac, char **av)
{
	...
	else
	{
		...
		else if (ac == 3 && (fd = open(av[2], O_RDONLY)) > 0 &&
				(op = options(av[1])) != NULL)
