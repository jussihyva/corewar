...
int		op_sti(t_game *game, int place, t_carriage *c)
{
	...
	val1 %= IDX_MOD;
	game->arena[ft_place(place + val1)] = (c->reg[ty[0][1] - 1] >> 24)
		& 0xFF;
	game->arena[ft_place(place + val1 + 1)] = (c->reg[ty[0][1] - 1] >> 16)
		& 0xFF;
	game->arena[ft_place(place + val1 + 2)] = (c->reg[ty[0][1] - 1] >> 8)
		& 0xFF;
	game->arena[ft_place(place + val1 + 3)] = c->reg[ty[0][1] - 1] & 0xFF;
	return (read_game_param(game, place, c->size, ty));
}
